...
export function LoginPage() {
  ...
  const throttledLogin = useCallback(
    throttle(async (email: string, password: string) => {
      setError("");
      if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (adminLoading) return; // Prevent submission if already loading
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    if (success) {
      setResetSent(true);
      setCurrentView("otp"); // Auto-switch to OTP view
    }
  };
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (formData.password !== otp) {
      setError("OTP does not match");
      return;
    }
    const success = await resetPassword(formData.email, otp, formData.password);
    if (success) {

  ...
  try {
    ...
    if (origin && destination && departureDate) {
      ...
      if (tripType === 'roundtrip' && returnDate) {
        ...
        const schedules = await scheduleService.searchSchedulesEnhanced(
          origin as string,
        ...
        const schedules = await scheduleService.findSchedulesByOriginDestination(
          origin as string,
          destination as string,
          departureDate as string,
          isReturnBool
...
export async function regenerateSeats(req: Request, res: Response) {
  try {
    const scheduleId = Number(req.params.id);
    if (isNaN(scheduleId)) {
      return res.status(400).json({ error: "Invalid schedule ID" });
    ...
    const seatCount =
      await scheduleService.regenerateSeatsForSchedule(scheduleId);
    res.json({
...
export async function forceDeleteSchedule(req: Request, res: Response) {
  try {
    const scheduleId = Number(req.params.id);
    if (isNaN(scheduleId)) {
      return res.status(400).json({ error: "Invalid schedule ID" });

...
// Interface for optimized ticket data
interface OptimizedTicketData {
  tn: string; // ticketNumber
  oid: number; // orderId
  u: {
    id: number;
    n: string; // name
...
export function generateOptimizedTicketData(data: {
  ...
  bus: { id: number; name: string; layoutType: string };
  schedule: {
    id: number;
    departure: Date;
    isReturn: boolean;
...
export async function generateTicketQRCode(
  ticketData: OptimizedTicketData
): Promise<string> {
  try {
    ...
    console.log(
      "Ticket QR code generated successfully for ticket:",
      ticketData.tn
    );

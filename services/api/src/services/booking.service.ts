...
import {
  generateOptimizedTicketData,
  generateTicketNumber,
  generateTicketQRCode,
} from "./ticket.service";
import { sendTicketEmail, sendCancellationEmail } from "./email.service";
import { format } from "date-fns";
import { AdminCreateBookingInput, PassengerDetails } from "@ebusewa/common";
...
interface BookingSeatsData {
  ...
  // Individual passenger details for each seat
  passengers?: PassengerDetails[];
}

interface MultiSeatBookingRequest {
  scheduleId: number;
  mainBooker: {
    userId: number;
    name: string;
...
export async function bookSeats(
  ...
  try {
    console.log(
      `Booking seats for user ${userId}, schedule ${scheduleId}, seats:`,
      seats
    );
    ...
    const seatValidations = await Promise.all(
      seats.map(async (seatNumber: string) => {
        ...
        if (!seat) {
          throw new Error(
            `Seat ${seatNumber} does not exist for schedule ${scheduleId}`
          );
        }
    ...
    try {
      result = await prisma.$transaction(
        async (tx: Prisma.TransactionClient) => {
          ...
          const seatsToBook = await Promise.all(
            seats.map(async (seatNumber: string) => {
              const seat = seatValidations.find(
      ...
      if (
        ...
        if (prismaError.meta?.target?.includes("seatId")) {
          throw new Error(
            "One or more seats are no longer available. Please try again with different seats."
          );
        }
...
export async function bookSeatsForUser(
  adminId: number,
  ...
  try {
    ...
    let customerId: number | undefined = data.userId;
    ...
    // Validate all seats exist and are available
    const seatValidations = await Promise.all(
      data.seatNumbers.map(async (seatNumber) => {
        const seat = await prisma.seat.findUnique({
          where: {
        ...
        if (!seat) {
          throw new Error(
            `Seat ${seatNumber} does not exist for schedule ${data.scheduleId}`
          );
        }
    ...
    try {
      result = await prisma.$transaction(
        async (tx: Prisma.TransactionClient) => {
          ...
          // Double-check seat availability within transaction
          const seatsToBook = await Promise.all(
            data.seatNumbers.map(async (seatNumber: string) => {
              const seat = seatValidations.find(
                (s) => s.seatNumber === seatNumber
              );
          ...
          const bookingData = seatsToBook.map(({ seat, seatNumber }) => {
            ...
            const passengerDetails = data.passengers?.[
              data.seatNumbers.indexOf(seatNumber)
            ] || {
    ...
    console.log(
      `Successfully booked ${data.seatNumbers.length} seats for customer ${customerId}`
    );
    ...
    if (
      ...
      throw new Error(
        "Missing required fields for QR code generation. Please ensure ticket number, customer name, and phone are provided."
      );
    }
    ...
    try {
      ...
      await sendTicketEmail({
        ...
        qrCodeData: (updatedTicket?.qrCode || "").replace(
...
export async function listAllBookingsWithPagination(params: {
  ...
  try {
    ...
    if (status) {
      where.status = status as "BOOKED" | "CANCELLED" | "COMPLETED";
    }
...
export async function cancelBooking(
  ...
  try {
    ...
    if (
      today.getFullYear() === travelDate.getFullYear() &&
    ...
    try {
      ...
      await sendCancellationEmail({
        ...
        qrCodeData:
          booking.order?.ticket?.qrCode?.replace(
            /^data:image\/png;base64,/,
            ""
          ) || undefined,
...
export async function resetSeatStatus(scheduleId: number) {
  try {
    ...
    if (orphanedBookings.length > 0) {
      console.log(
        `Found ${orphanedBookings.length} orphaned bookings, cleaning up...`
      );
...
export async function updateBookingStatus(
  ...
  try {
    console.log(
      `Updating booking ${bookingId} status to ${status} by admin ${adminId}`
    );
    ...
    const updatedBooking = await prisma.booking.update({
      where: { id: bookingId },
      data: { status: status as "BOOKED" | "CANCELLED" | "COMPLETED" },
      include: {
        user: true,
        schedule: { include: { route: true } },
        seat: true,
    ...
    console.log(
      `Successfully updated booking ${bookingId} status to ${status}`
    );
...
export async function adminCancelBooking(
  ...
  try {
    ...
    console.log(
      `Successfully cancelled booking ${bookingId} by admin ${adminId}`
    );
...
export async function createMultiSeatBooking(data: MultiSeatBookingRequest) {
  // Validate schedule
  const schedule = await prisma.schedule.findUnique({
    where: { id: data.scheduleId },
    include: { route: true, bus: true },
  ...
  return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    const bookings = await Promise.all(
      data.seats.map(async (seatObj) => {
        const seat = seatRecords.find(
          (s) => s.seatNumber === seatObj.seatNumber
        );
        if (!seat) throw new Error(`Seat ${seatObj.seatNumber} not found`);
        ...
        if (existingBooking) {
          throw new Error(
            `Seat ${seatObj.seatNumber} is already booked (concurrent)`
          );
        }
    ...
    const ticketData = generateOptimizedTicketData({
      ...
      seatNumbers: bookings.map(
        (b) => seatRecords.find((s) => s.id === b.seatId)?.seatNumber ?? ""
      ),
...
export async function createMultiSeatBookingWithEmail(
  data: MultiSeatBookingRequest
) {
  try {
    ...
    try {
      ...
      await sendTicketEmail({
        ...
        paymentMethod: data.paymentMethod || "PENDING",
        qrCodeData: (result.ticket.qrCode || "").replace(
          /^data:image\/png;base64,/,
          ""
        ),
      });
...
export async function createMultiSeatBookingForUser(
  adminId: number,
  data: AdminCreateBookingInput
) {
  try {
    ...
    const onwardResult = await prisma.$transaction(
      async (tx: Prisma.TransactionClient) => {
    ...
    try {
      ...
      await sendTicketEmail({
        customerName: data.bookerName,
        customerEmail:
          data.bookerEmail || `${data.bookerPhone}@guest.ebusewa.com`,
        customerPhone: data.bookerPhone,
        ticketNumber: onwardResult.ticketNumber,
        seatNumbers: data.passengers.map((p) => p.seatNumber),
        departureTime: format(departureDate, "HH:mm"),
        ...
        totalAmount: schedule.fare * data.passengers.length,
        paymentMethod: (data.paymentMethod || "CASH").toUpperCase(),
        qrCodeData: (onwardQrCode || "").replace(
          /^data:image\/png;base64,/,
          ""
        ),
      });
    ...
    if (
      ...
      if (!returnSchedule) throw new Error("Return schedule not found");
      ...
      returnResult = await prisma.$transaction(
        async (tx: Prisma.TransactionClient) => {
          const order = await tx.order.create({ data: { userId: customerId } });
          await Promise.all(
            data.returnPassengers!.map(async (passenger) => {
              const seat = returnSeatRecords.find(
                (s) => s.seatNumber === passenger.seatNumber
              );
              if (!seat)
                throw new Error(
                  `Return seat ${passenger.seatNumber} not found`
                );
              await tx.seat.update({
                where: { id: seat.id },
                data: { isBooked: true },
          ...
          const totalAmount =
            returnSchedule.fare * data.returnPassengers!.length;
          const paymentMethod = data.paymentMethod || "CASH";
          await tx.payment.create({
            data: {
              userId: customerId,
      ...
      const returnTicketData = generateOptimizedTicketData({
        ...
        paymentMethod: (data.paymentMethod || "CASH").toUpperCase(),
        totalAmount:
          returnResult.returnSchedule.fare * data.returnPassengers.length,
      });
      ...
      try {
        ...
        await sendTicketEmail({
          ...
          totalAmount:
            returnResult.returnSchedule.fare * data.returnPassengers.length,
          paymentMethod: (data.paymentMethod || "CASH").toUpperCase(),
          qrCodeData: (returnQrCode || "").replace(
            /^data:image\/png;base64,/,
            ""
          ),
        });
...
export async function removeSeatFromBooking(
  bookingId: number,
  seatNumber: string,
  adminId: number
) {
  ...
  if (!order) throw new Error("Order not found for this booking");

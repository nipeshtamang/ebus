...
export async function getSummaryReport(filters = {}) {
    try {
        ...
        // Group by route
        const revenueByRouteGrouped = revenueByRoute.reduce((acc, payment) => {
            const routeId = payment.booking?.schedule?.route?.id;
            const routeName = payment.booking?.schedule?.route?.name || "Unknown Route";
            if (routeId) {
                if (!acc[routeId]) {
        ...
        const bookingsByRouteGrouped = bookingsByRoute.reduce((acc, booking) => {
        ...
        const userWhere = {};
        if (dateFrom)
            userWhere.createdAt = { gte: dateFrom };
        if (dateTo) {
        ...
        const occupancyRateFormatted = occupancyRate.map((bus) => {
            const totalSeats = bus.schedules.reduce((sum, schedule) => sum + schedule.seats.length, 0);
            const bookedSeats = bus.schedules.reduce((sum, schedule) => sum +
                schedule.seats.filter((seat) => seat.booking && seat.booking.deletedAt === null).length, 0);
            return {
                busId: bus.id,
                busName: bus.name,
